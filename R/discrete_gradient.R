...
scale_colour_discrete_gradient <- function(..., colours, bins, na.value = "grey50", guide = "colourbar", aesthetics = c("colour"), colors)  {
...
scale_fill_discrete_gradient <- function(..., colours, bins, na.value = "grey50", guide = "colourbar", aesthetics = c("fill"), colors)  {
  colours <- if (missing(colours)) 
    colors
  else colours
  continuous_scale(
